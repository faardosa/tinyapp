...
app.get("/urls", (req, res) => {
  ...
  if (!userId) {
    res.send(
      "Error! You are not logged in! <a href = '/login' > Click here to login</a>"
    );
    return;
  }
...
app.get("/u/:shortURL", (req, res) => {
  ...
  if (!url) {
    res.send("Error the url does not exist");
  } else {
...
app.post("/login", (req, res) => {
  ...
  if (!password || !email)
    return res.status(403).send("Please put valid email and password");
...
app.post("/register", (req, res) => {
  // make a post for the registration, this is creating a new user
